...
extension VotePostDetailViewController {
    ...
    func fetchPosts(page: Int, completion: @escaping ([VoteComment]?, Error?) -> Void) {
        // 투표를 조회해서 찬성, 반대 수 가져오기
        VoteStatusCheck()
        agreeCountLabel.text = "찬성: \(agreeCount)"
        disagreeCountLabel.text = "반대: \(disagreeCount)"
            ...
            do {
                ...
                if let result = json?["result"] as? [String: Any], let comments = result["commentList"] as? [[String: Any]] {
                    ...
                    for comment in comments {
                        ...
                        {
                            let post = VoteComment(day: day, commentId: commentId, comment: commentText, commentIsMine: isMine)
                            posts.append(post)
                        }
                    }
